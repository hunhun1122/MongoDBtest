 ...
 * @since 2009-07-15
 */
public class GWJPUtils {

	private static Log log = LogFactory.getLog(GWJPUtils.class);
...
//	private static PropertiesHelper pHelper = PropertiesFactory.getPropertiesHelper(PropertiesFile.G4);
	...
	public static boolean isTheStyle(String pStr, String pStyle) {
		for (int i = 0; i < pStr.length(); i++) {
			...
			if (pStyle.equals(GWJPConstants.S_STYLE_N)) {
				if (!Character.isDigit(c))
					return false;
			} else if (pStyle.equals(GWJPConstants.S_STYLE_L)) {
				if (!Character.isLetter(c))
					return false;
			} else if (pStyle.equals(GWJPConstants.S_STYLE_NL)) {
	...
	public static String getGBK(String pString){
		...
		try {
			...
			log.error(GWJPConstants.Exception_Head + "不支持的字符集编码");
			e.printStackTrace();
		}
		return pString;
	}
